...
async function getSavedRecipeIds() {
  try {
    ...
    // Extract meal IDs from recipe names by matching against allMeals
    const savedNames = new Set(
      items.map((item) => item.recipeName.toLowerCase()),
    );
    return new Set(
      allMeals
        .filter((meal) => savedNames.has(meal.strMeal.toLowerCase()))
