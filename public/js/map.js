// AI was used to assist in some of the features below.

// Global State Variables

// Scan radius in metres (default 500 m)
let pingRadius = 500; 
// How far (km) along a bus/skytrain route to follow (default 1 km)
let transitExpansion = 1; 
// Radius (m) around a transit stop to search for stores
let secondaryPingRadius = 100; 

// All green grocery store markers currently on the map
let currentMarkers = []; 
// Relay mode markers (yellow/purple/split)
let relayMarkers = []; 
// setTimeout IDs used by relay — kept so they can be cancelled
let relayTimeouts = []; 
 // Lat/lon of stores added by relay mode
let relayStoreLocations = [];
// Mapbox IDs of every store already on the map (prevents duplicates)
let addedStoreIds = new Set(); 
// Tracks which IDs relay specifically added (so clearing relay can undo just those)
let relayAddedStoreIds = new Set(); 
// Lat/lon of stores added by the primary scan (not relay)
let groceryStoreLocations = []; 
// Incremented each time runRelay() starts; lets stale async calls bail out early
let relayGeneration = 0; 
// Most recent GPS longitude (null until first fix)
let lastKnownLon = null; 
 // Most recent GPS latitude
let lastKnownLat = null;

// Feature toggle flags each is flipped by the panel checkboxes
// Whether to show bus stops
let busEnabled = false; 
// Whether to show SkyTrain stops
let skytrainEnabled = false; 
// Whether "Transit Relay" mode is active
...
function createCircleGeoJSON(lon, lat, radiusMeters)
{
  ...
  {
    // Evenly space points around 360°
...
function updateUserScanCircle()
{
  ...
  map
    .getSource("scan-circle-user")
    .setData(createCircleGeoJSON(lastKnownLon, lastKnownLat, pingRadius));
}
...
function updateCustomScanCircle()
{
  ...
  {
    map
      .getSource("scan-circle-custom")
      .setData(createCircleGeoJSON(customScanLon, customScanLat, pingRadius));
  }
}
...
map.on("load", () =>
{
  // Register the circle drawing layers
  initScanCircleLayers(); 
  // Ask the browser for the user location immediately
  geolocate.trigger(); 
});
...
// run can rediscover those stores fresh.
...
function parseOSMHours(raw)
{
  ...
  // Short OSM day codes into full names
  const dayMap =
  {
    Mo: "Mon",
    Tu: "Tue",
  ...
  try
  {
    ...
    const lines = segments.map((seg) =>
    {
      ...
      // Return the segment unchanged if it doesn't match either pattern
      return seg; 
    });
  ...
  {
    // If anything fails show the raw string rather than crashing
    return raw; 
  }
}
...
// Tries to match by name and falls back to the closest result.
async function fetchOSMDetails(name, lat, lon)
{
  ...
  {
    // Network/parsing error then return null silently
    return null; 
  }
}
...
function fetchGroceryStores(lon, lat)
{
  clearMarkers();
  clearRelayMarkers();
  // Reset dedup tracking for this fresh scan
  addedStoreIds.clear(); 

  // Fire all category searches in parallel, then combine results
  Promise.all(CATEGORIES.map((cat) => fetchMapboxCategory(cat, lon, lat)))
    .then((results) =>
    {
      ...
      // If relay is on relay onStopComplete handles bus/skytrain (after
    ...
    .then((results) =>
    {
      ...
      allFeatures
        .filter((store) =>
        {
          ...
          const id = store.properties.mapbox_id;
          return (
            haversineKm(lat, lon, storeLat, storeLon) <=
              secondaryPingRadius / 1000 &&
            // Skip stores already on the map
        ...
        .forEach((store) =>
        {
          ...
          else
          {
            ...
            {
              feature: store,
              transitTypes: new Set([transitType]),
            });
          }
        });
...
function renderRelayStores(collector)
{
  collector.forEach(({ feature, transitTypes }) =>
  {
    ...
    if (hasBus && hasTrain)
    {
      ...
      {
        element: createSplitMarkerElement(),
        anchor: "bottom",
      })
        .setLngLat([storeLon, storeLat])
        .setPopup(popup)
...
function runRelay(lon, lat)
{
  ...
  [
    ...
    .then(([busStopsRaw, skytrainStopsRaw]) =>
    {
      ...
      busStopsRaw.forEach((s) =>
      {
        s._transitType = "bus";
      });
      skytrainStopsRaw.forEach((s) =>
      {
        s._transitType = "skytrain";
      });
      ...
      allStops.forEach((stop) =>
      {
        const onNearbyRoute = Object.keys(stop.routes).some((r) =>
          nearbyRoutes.has(r),
        );
        const withinExpansion =
          haversineKm(lat, lon, stop.lat, stop.lon) <= transitExpansion;
      ...
      stopsToFetch.forEach((stop, i) =>
      {
        ...
        // Keep the timeout ID so clearRelayMarkers() can cancel it
...
async function saveLocation(button, name, address)
{
  ...
  // Show server response message to the user
  alert(result.message);
}

// Guards against triggering a full scan on every GPS update
...
busToggle.addEventListener("change", () =>
{
  ...
  if (!busEnabled)
  {
    // Remove all blue bus-stop pins
    clearBusMarkers();
  }
  else if (lastKnownLon !== null)
  {
    // Show bus stops immediately
...
foodBankToggle.addEventListener("change", () =>
{
  ...
  if (foodBanksEnabled)
  {
    ...
    {
      // Data already loaded just show the markers
      renderFoodBanks();
    }
    else
    {
...
applyBtn.addEventListener("click", () =>
{
  ...
  const expansionVal = parseFloat(
    document.getElementById("expansion-input").value,
  );
  const secondaryVal = parseFloat(
  ...
  if (!isNaN(expansionVal) && expansionVal > 0)
    transitExpansion = Math.min(expansionVal, 5);
  if (!isNaN(secondaryVal) && secondaryVal > 0)
    secondaryPingRadius = Math.min(secondaryVal, 300);
  ...
  {
    // Trigger a fresh scan with the new settings
...
// All blue bus stop pins currently on the map
let busMarkers = [];
// All orange SkyTrain stop pins currently on the map
...
function isNearGroceryStore(stopLat, stopLon)
{
  ...
  return allStores.some(
    (store) =>
      haversineKm(stopLat, stopLon, store.lat, store.lon) <=
      secondaryPingRadius / 1000,
  );
}
...
function deduplicateStops(stops, userLat, userLon)
{
  ...
  for (let i = 0; i < stops.length; i++)
  {
    ...
    for (let j = i + 1; j < stops.length; j++)
    {
      ...
      if (
        haversineKm(stops[i].lat, stops[i].lon, stops[j].lat, stops[j].lon) >
        DEDUP_KM
      )
        continue;
      ...
      const sharesRoute = Object.keys(stops[j].routes).some((r) =>
        routesI.has(r),
      );
      if (!sharesRoute) continue;

      // Remove the farther stop UNLESS it is near a grocery store
...
function deduplicateByStoreOverlap(stops, userLat, userLon)
{
  ...
  for (let i = 0; i < stops.length; i++)
  {
    ...
    // Stop doesn't cover any known store keep it
    if (keyI === "") continue;
    ...
    for (let j = i + 1; j < stops.length; j++)
    {
      ...
      // Different store coverage keep both
...
function fetchBusStops(lon, lat)
{
  ...
  fetch("/data/busStops.json")
    ...
    .then((stops) =>
    {
      ...
      // No bus routes nearby nothing to show
      if (pingedRoutes.size === 0) return;

      // Step 2: expand along those routes up to transitExpansion km
      const expanded = stops.filter((stop) =>
      {
        const onPingedRoute = Object.keys(stop.routes).some((r) =>
          pingedRoutes.has(r),
        );
        const withinExpansion =
          haversineKm(lat, lon, stop.lat, stop.lon) <= transitExpansion;
      ...
      const deduped50m = deduplicateStops(expanded, lat, lon);
      const stopsToShow = deduplicateByStoreOverlap(
        deduped50m,
        lat,
        lon,
      ).filter(
      ...
      {
        const visibleRoutes = Object.entries(stop.routes).filter(([r]) =>
          pingedRoutes.has(r),
        );
...
// All dark orange food bank pins on the map
let foodBankMarkers = [];
// Whether the food bank toggle is on

let foodBanksEnabled = false;
// Cached food bank JSON (loaded once on first toggle)
let foodBankData = [];
...
// The draggable red Mapbox Marker (null when not active)
let customMarker = null;
// Longitude of the custom scan pin (null when not active)
let customScanLon = null;
// Latitude of the custom scan pin
...
{
  // Can't enter if GPS hasn't fired
...
function exitCustomMode()
{
  ...
  // Clears the custom circle (customScanLon is now null)
  updateCustomScanCircle();
...
customBtn.addEventListener("click", () =>
{
  if (customMarker)
  {
    // Pin is active remove it
    exitCustomMode();
  }
  else
  {
    // Pin is not active place it
    enterCustomMode();
  }
});
...
(function initAIShopping()
{
  ...
  function waitForStores()
  {
    if (allScannedStoreData.length > 0)
    {
      ...
      suggestBtn.textContent =
        "Suggest Best Stores (" + allScannedStoreData.length + " found)";
    }
    else
    {
  ...
  async function runAISuggestion()
  {
    ...
    {
      resultsList.innerHTML =
        '<div class="ai-loading-msg" style="color:#b91c1c">Error: ' +
        err.message +
        "</div>";
    }
    finally
  ...
  function renderSuggestions(suggestions)
  {
    ...
    {
      resultsList.innerHTML =
        '<div class="ai-loading-msg">No suggestions found.</div>';
      return;
    }
